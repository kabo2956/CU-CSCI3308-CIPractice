...
START_TEST(tc_coord_2d_area_triangle){
	coord_2d_t a;
	coord_2d_t b;
	coord_2d_t c;
	...
	c.x = 1;
	c.y = 0;
	ck_assert(abs(coord_2d_area_triangle(&a, &b, &c)-0.5) < 0.01);
	...
	c.x = 4;
	c.y = -4;
	ck_assert(abs(coord_2d_area_triangle(&a, &b, &c)-8) < 0.01 );
	...
	c.x = 0;
	c.y = 1;
	ck_assert(abs(coord_2d_area_triangle(&a, &b, &c)) < 0.01);
}
END_TEST
...
Suite* coord_2d_suite(void)
{
    ...
    tcase_add_test(tc_2d_area_triangle, tc_coord_2d_area_triangle);
    /* Add Cases to Suite */
